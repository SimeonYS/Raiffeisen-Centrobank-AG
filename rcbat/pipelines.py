...
class RcbatPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS articles
                    (date text, title text, subtitle text, link text, content text)""")
    ...
    def process_item(self, item, spider):
        ...
        # Insert values
        self.c.execute("INSERT INTO articles (date, title, subtitle, link, content)"
                       "VALUES (?,?,?,?,?)", (item.get('date'), item.get('title'), item.get('subtitle'), item.get('link'), item.get('content')))
